...
{
    public class IngressWatcher : IResourceWatcher
    {
        ...
        public IngressWatcher(KubernetesClientService kubernetesClientService, IHubContext<KubernetesHub> hubContext)
        {
        ...
        public async Task StartWatchingAsync(CancellationToken cancellationToken, string namespaceName)
        {
            ...
            try
            {
                ...
                {
                    await _hubContext.Clients.Group(namespaceName).SendAsync("ReceiveIngressUpdate", new { Type = type.ToString(), DeploymentName = item.Metadata.Name, item });
                }
            }
